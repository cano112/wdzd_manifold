...
t0 = time()
num = 12000#len(data)
# lle = manifold.LocallyLinearEmbedding(n_neighbors, 2, method=method)
isomap = manifold.Isomap(n_neighbors=n_neighbors, n_components=2)
trans_data = isomap.fit_transform(data[:num]).T
t1 = time()
fig = plt.figure(figsize=(15, 8))
ax = fig.add_subplot(111)
plt.scatter(trans_data[0], trans_data[1], c=np.random.rand(num))
plt.title("Isomap ({:2.3g} ".format(t1 - t0))
ax.xaxis.set_major_formatter(NullFormatter())
ax.yaxis.set_major_formatter(NullFormatter())
plt.axis('tight')
# plt.savefig('isomap_defaults.png')
print("Saved figure for Isomap in time {:2.3g}.".format(t1-t0))
plt.show()
plt.clf()
